...
import { type SymmetricCipher } from '../ciphers.js';
...
export class SplitKey {
  ...
  async generateKey(): Promise<KeyInfo> {
    ...
    const unwrappedKeyBinary = Binary.fromArrayBuffer(unwrappedKey.buffer);
  ...
  async generateIvBinary(): Promise<Binary> {
    ...
    return Binary.fromArrayBuffer(iv.buffer);
  }

  async write(policy: Policy, kek: KeyInfo): Promise<EncryptionInformation> {
    const algorithm = this.cipher.name;
    if (!algorithm) {
      throw new Error('Uninitialized cipher type');
    }
    const keyAccessObjects = await this.getKeyAccessObjects(policy, kek);
    ...
    return {
      ...
      method: {
        algorithm,
        isStreamable: false,
        iv: base64.encodeArrayBuffer(kek.unwrappedKeyIvBinary.asArrayBuffer()),
      },
